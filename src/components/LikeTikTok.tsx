...
import { ThumbsUp, Clock, Award, AlertCircle, Sparkles, X, ShieldAlert, Music } from 'lucide-react';
import TaskVerificationOverlay from './TaskVerificationOverlay';
import { SupportedLanguages } from '../lib/translations';

interface LikeTikTokProps {
  user: User;
  onPointsEarned: () => void;
  lang?: string;
}

const tiktokLikeTranslations: Record<SupportedLanguages, Record<string, string>> = {
  ar: {
    availableTitle: 'متاح حالياً: {count} حملة',
    mainTitle: 'تبادل إعجابات ولايكات تيك توك',
    mainDesc: 'ضع اللايك والتفاعل اللائق على فيديوهات تيك توك الخاصة بالأعضاء لكسب رصيد مكافأة متكامل. يشترط الاحتفاظ بنافذة المشاهدة لمدة 15 ثانية للتأكيد الفريد.',
    noCampaigns: 'لا توجد حملات لايك تيك توك حالياً',
    noCampaignsDesc: 'عد لاحقاً لرؤية حسابات تيك توك الجديدة أو ابدأ بحسابك الآن لتصل لملايين المتابعين!',
    yourCampaign: 'حملتك الإعلانية ⭐️',
    rewardPoints: '{reward} نقطة',
    requiredTime: '15 ثانية انتظار',
    startLiking: 'إعجاب بالفيديو واكسب {reward} نقطة',
    testAndPreview: 'اختبر حملتك ومعاينة الرابط',
    prevBtn: 'السابق &rarr;',
    nextBtn: '&larr; التالي',
    pageIndicator: 'المجموعة {current} من {total}',
    modalTitle: 'مهمة لايك تيك توك',
    modalDesc: 'تبادل لايك وإعجابات تيك توك. يرجى الانتظار وتكمل التفاعل بالنافذة المفتوحة.',
  ...
  en: {
    ...
    mainTitle: 'TikTok Like Exchanges',
    mainDesc: 'Earn points by liking other users\' TikTok videos. Please stay on the page for 15 seconds to confirm your reaction.',
    noCampaigns: 'No TikTok Like Campaigns Available',
    noCampaignsDesc: 'Create your own campaign to get real likes!',
    yourCampaign: 'Your Campaign ⭐️',
    rewardPoints: '{reward} Points',
    requiredTime: '15 seconds stay',
    startLiking: 'Like Video & earn {reward} Points',
    testAndPreview: 'Test campaign & preview link',
    prevBtn: 'Previous &rarr;',
    nextBtn: '&larr; Next',
    pageIndicator: 'Page {current} of {total}',
    modalTitle: 'TikTok Like Task',
    modalDesc: 'TikTok like and interaction exchange. Please wait and complete the action in the opened window.',
  ...
  fr: {
    ...
    mainTitle: 'Échanges de Likes TikTok',
    mainDesc: 'Aimez des vidéos de créateurs de la communauté TikTok pour cumuler des points. S\'il vous plaît restez 15 sec pour l\'audit de sécurité.',
    noCampaigns: 'Aucun objectif de Like TikTok disponible',
    noCampaignsDesc: 'Boostez vos vidéos en créant votre premiere campagne de likes !',
    yourCampaign: 'Votre campagne ⭐️',
    rewardPoints: '{reward} Points',
    requiredTime: '15 secondes',
    startLiking: 'Liker la vidéo & Gagner {reward} Pts',
    testAndPreview: 'Vérifier ma campagne & Aperçu',
    prevBtn: 'Précédent &rarr;',
    nextBtn: '&larr; Suivant',
    pageIndicator: 'Page {current} de {total}',
    modalTitle: 'Liker sur TikTok',
    modalDesc: 'Échange de réactions TikTok. Veuillez patienter dans la fenêtre externe ouverte.',
    ...
    verificationError: 'Erreur de validation technique. Campagne déjà validée par votre profil.'
  },
  es: {
    availableTitle: 'Disponibles: {count} de campañas',
    mainTitle: 'Intercambios de Me Gusta TikTok',
    mainDesc: 'Marque me gusta en vídeos de la comunidad de TikTok para ganar saldo. Es obligatorio permanecer 15 segundos en navegación.',
    noCampaigns: 'No hay campañas de Me Gusta TikTok activas',
    ...
    modalTitle: 'Acción Reacción TikTok',
    modalDesc: 'Intercambio de likes de vídeos TikTok. Complete la interacción en el destino abierto.',
    ...
    startAlert: '⚠️ Oprima el botón del centro para abrir el vídeo de TikTok objetivo del anunciante',
    verificationError: 'Surgió un error. Su interacción no puede ser validada nuevamente.'
  }
};

export default function LikeTikTok({ user, onPointsEarned, lang = 'en' }: LikeTikTokProps) {
  ...
  const t = tiktokLikeTranslations[activeLang];
  ...
  const handleVerificationSuccess = (profileHandle: string) => {
    const reward = activeCamp?.rewardPerAction || 40; // TikTok follow reward: 40 points
    const success = db.recordInteraction(user.uid, activeCamp!.id, 'tiktok_like', reward, profileHandle);
  ...
  return (
    <div className={`w-full font-sans ${activeLang === 'ar' ? 'text-right' : 'text-left'}`} dir={activeLang === 'ar' ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between border-b border-slate-800 pb-5 mb-6 flex-row">
        ...
        <h2 className="text-xl md:text-2xl font-black text-white flex items-center gap-2 flex-row border-slate-700">
          {t.mainTitle}
          <ThumbsUp className="w-6 h-6 text-cyan-405 text-cyan-400 animate-pulse" />
        </h2>
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {currentCampaigns.map((camp) => (
              <div id={`tiktok-like-card-${camp.id}`} key={camp.id} className="bg-slate-900 border border-slate-800 hover:border-cyan-500/30 rounded-2xl p-5 flex flex-col justify-between gap-4 transition-all hover:translate-y-[-2px] relative overflow-hidden">
                <div className="absolute top-0 right-0 w-1.5 h-full bg-gradient-to-b from-cyan-400 to-rose-500"></div>
                <div>
                ...
                <button
                  id={`tiktok-like-start-${camp.id}`}
                  onClick={() => handleStart(camp)}
                  className="w-full py-2.5 bg-slate-950 hover:bg-cyan-600 hover:text-white border border-slate-800 hover:border-cyan-500 rounded-xl font-semibold text-xs text-cyan-400 transition flex items-center justify-center gap-1.5 cursor-pointer flex-row"
                >
                  <ThumbsUp className="w-4 h-4 text-cyan-400 hover:text-white" />
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-6 bg-slate-900 border border-slate-800 p-4 rounded-2xl flex-row">
              <button
                ...
                className="px-4 py-2 text-xs font-bold text-slate-300 bg-slate-950 border border-slate-850 hover:border-cyan-500/30 rounded-xl hover:bg-slate-855 hover:text-white transition disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
              >
                <span>{t.prevBtn}</span>
              </button>
              ...
              <button
                ...
                className="px-4 py-2 text-xs font-bold text-slate-300 bg-slate-950 border border-slate-850 hover:border-cyan-500/30 rounded-xl hover:bg-slate-850 hover:text-white transition disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
              >
                <span>{t.nextBtn}</span>
              </button>
            </div>
      ...
      {activeCamp && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-[#0f141c] border border-slate-800/80 rounded-3xl p-6 md:p-8 flex flex-col items-center relative shadow-2xl overflow-hidden">
            ...
            <div className="absolute -top-10 -left-10 w-32 h-32 bg-cyan-500/10 rounded-full blur-3xl pointer-events-none opacity-20"></div>
            ...
            <div className="inline-flex p-5 rounded-full mb-4 bg-gradient-to-tr from-cyan-555 to-rose-655 text-cyan-400 border border-cyan-500/20 shadow-lg shadow-cyan-500/10 animate-pulse font-semibold">
            ...
            {/* State alerts */}
            {alertMsg && (
              <div className="w-full mb-4 text-[11px] text-rose-455 font-bold bg-rose-500/10 border border-rose-500/15 py-2.5 px-3 rounded-xl flex items-center gap-2 text-right flex-row">
