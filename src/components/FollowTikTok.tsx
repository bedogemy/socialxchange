...
import { UserPlus, Clock, Award, AlertCircle, Sparkles, X, ShieldAlert, Music } from 'lucide-react';
import TaskVerificationOverlay from './TaskVerificationOverlay';
import { SupportedLanguages } from '../lib/translations';

interface FollowTikTokProps {
  user: User;
  onPointsEarned: () => void;
  lang?: string;
}

const tiktokFollowTranslations: Record<SupportedLanguages, Record<string, string>> = {
  ar: {
    availableTitle: 'متاح حالياً: {count} حملة',
    mainTitle: 'تبادل فولو ومتابعات تيك توك',
    mainDesc: 'قم بزيادة رصيدك عبر عمل متابعات حقيقية لحسابات تيك توك الخاصة بأعضاء المنصة الآخرين. يشترط الدعم بقاء التصفح نشطاً لـ 15 ثانية لتسجيل الإجراء بأمان.',
    noCampaigns: 'لا توجد حملات فولو تيك توك حالياً',
    ...
    startLiking: 'متابعة الحساب واكسب {reward} نقطة',
    testAndPreview: 'اختبر حملتك ومعاينة الرابط',
    prevBtn: 'السابق &rarr;',
    nextBtn: '&larr; التالي',
    pageIndicator: 'المجموعة {current} من {total}',
    modalTitle: 'مهمة متابعة تيك توك',
    modalDesc: 'تبادل فولو ومتابعات تيك توك. يرجى الانتظار وتكمل التفاعل بالنافذة المفتوحة.',
  ...
  en: {
    ...
    mainTitle: 'TikTok Follows Exchange',
    mainDesc: 'Earn points by following other users\' TikTok profiles. Please stay on the page for 15 seconds to confirm your subscription.',
    noCampaigns: 'No TikTok Follow Campaigns Available',
    noCampaignsDesc: 'Create your own campaign to get real followers!',
    yourCampaign: 'Your Campaign ⭐️',
    rewardPoints: '{reward} Points',
    requiredTime: '15 seconds stay',
    startLiking: 'Follow Account & earn {reward} Points',
    testAndPreview: 'Test campaign & preview link',
    prevBtn: 'Previous &rarr;',
    nextBtn: '&larr; Next',
    pageIndicator: 'Page {current} of {total}',
    modalTitle: 'TikTok Follow Task',
    modalDesc: 'TikTok follow and interaction exchange. Please wait and complete the action in the opened window.',
  ...
  fr: {
    ...
    mainTitle: 'Échanges d\'Abonnements TikTok',
    mainDesc: 'Suivez les comptes d\'autres créateurs sur TikTok prour amasser des points. Un délai de 15 sec de présence est instrumenté.',
    noCampaigns: 'Aucun objectif d\'Abonnement TikTok',
    noCampaignsDesc: 'Invitez votre propre audience en créant votre plan de followers TikTok !',
    yourCampaign: 'Votre campagne ⭐️',
    rewardPoints: '{reward} Points',
    requiredTime: '15 secondes',
    startLiking: 'Suivre le compte & Gagner {reward} Pts',
    testAndPreview: 'Vérifier ma campagne & Aperçu',
    prevBtn: 'Précédent &rarr;',
    nextBtn: '&larr; Suivant',
    pageIndicator: 'Page {current} de {total}',
    modalTitle: 'S\'abonner sur TikTok',
    modalDesc: 'Échange d\'abonnements TikTok. Veuillez patienter dans la fenêtre externe ouverte.',
  ...
  es: {
    ...
    mainTitle: 'Intercambios de Seguidores TikTok',
    mainDesc: 'Siga cuentas de la comunidad de TikTok para ganar saldo. Es obligatorio permanecer 15 segundos en navegación.',
    noCampaigns: 'No hay campañas de Seguidores TikTok activas',
    ...
    startLiking: 'Seguir perfil & Ganar {reward} Puntos',
    testAndPreview: 'Probar y Validar Campaña',
    prevBtn: 'Anterior &rarr;',
    nextBtn: '&larr; Siguiente',
    pageIndicator: 'Página {current} de {total}',
    modalTitle: 'Acción Seguir TikTok',
    modalDesc: 'Intercambio de seguidores TikTok. Complete la interacción en el destino abierto.',
    ...
    startAlert: '⚠️ Oprima el botón del centro para abrir la cuenta de TikTok objetivo del anunciante',
    verificationError: 'Surgió un error. Su interacción no puede ser validada nuevamente.'
  }
};

export default function FollowTikTok({ user, onPointsEarned, lang = 'en' }: FollowTikTokProps) {
  ...
  const t = tiktokFollowTranslations[activeLang];
  ...
  const handleVerificationSuccess = (profileHandle: string) => {
    ...
    const success = db.recordInteraction(user.uid, activeCamp!.id, 'tiktok_follow', reward, profileHandle);
    ...
    <div className={`w-full font-sans ${activeLang === 'ar' ? 'text-right' : 'text-left'}`} dir={activeLang === 'ar' ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between border-b border-slate-800 pb-5 mb-6 flex-row">
        ...
        <h2 className="text-xl md:text-2xl font-black text-white flex items-center gap-2 flex-row">
          {t.mainTitle}
          <UserPlus className="w-6 h-6 text-cyan-450 text-cyan-400 animate-pulse" />
        </h2>
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {currentCampaigns.map((camp) => (
              <div id={`tiktok-follow-card-${camp.id}`} key={camp.id} className="bg-slate-900 border border-slate-800 hover:border-cyan-500/30 rounded-2xl p-5 flex flex-col justify-between gap-4 transition-all hover:translate-y-[-2px] relative overflow-hidden">
                ...
                <button
                  id={`tiktok-follow-start-${camp.id}`}
                ...
                >
                  <UserPlus className="w-4 h-4 text-cyan-400 hover:text-white" />
