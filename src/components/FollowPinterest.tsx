...
import { UserPlus, Clock, Award, AlertCircle, Sparkles, X, Pin } from 'lucide-react';
import TaskVerificationOverlay from './TaskVerificationOverlay';
import { SupportedLanguages } from '../lib/translations';

interface FollowPinterestProps {
  user: User;
  onPointsEarned: () => void;
  lang?: string;
}

const pinFollowTranslations: Record<SupportedLanguages, Record<string, string>> = {
  ar: {
    availableTitle: 'متاح حالياً: {count} حملة',
    mainTitle: 'تبادل متابعات بنترست (Pinterest)',
    mainDesc: 'شارك بمتابعة حسابات وبوردات الأعضاء على بنترست لجمع نقاط مكافأة فورية بآمان ومصداقية. البقاء بالتصفح لـ 15 ثانية إلزامي.',
    noCampaigns: 'لا توجد حملات متابعة بنترست حالياً',
    noCampaignsDesc: 'أنشئ حملتك الترويجية الأولى لمتابعة بنترست وسيتفاعل معها آلاف الأعضاء فوراً!',
    ...
    modalTitle: 'مهمة متابعة بنترست',
    modalDesc: 'تبادل متابعات حسابات بنترست. يرجى الانتظار وتكمل التفاعل بالنافذة المفتوحة.',
  ...
  en: {
    ...
    mainTitle: 'Pinterest Follow Exchanges',
    mainDesc: 'Participate by following Pinterest profiles and boards of other members to collect instant points safely. Staying for 15 seconds is mandatory.',
    noCampaigns: 'No Pinterest follow campaigns currently',
    noCampaignsDesc: 'Create your first Pinterest follow campaign now and thousands of members will react to it immediately!',
    yourCampaign: 'Your Campaign ⭐️',
    rewardPoints: '{reward} Points',
    requiredTime: '15 seconds stay',
    startLiking: 'Follow Profil & earn {reward} Points',
    testAndPreview: 'Test campaign & preview link',
    prevBtn: 'Previous &rarr;',
    nextBtn: '&larr; Next',
    pageIndicator: 'Page {current} of {total}',
    modalTitle: 'Pinterest Follow Task',
    modalDesc: 'Pinterest profile follow exchange. Please wait and complete the action in the opened window.',
  ...
  fr: {
    ...
    mainTitle: 'Échanges d\'Abonnements Pinterest',
    mainDesc: 'Suivez les profils de membres sur Pinterest pour gagner des points. Un délai de présence de 15 secondes est requis.',
    noCampaigns: 'Aucun objectif d\'Abonnement Pinterest',
    noCampaignsDesc: 'Créez votre première campagne Pinterest Follow maintenant pour obtenir de l\'audience !',
    ...
    modalTitle: 'S\'abonner sur Pinterest',
    modalDesc: 'Échange d\'abonnements Pinterest. Veuillez patienter dans la fenêtre externe ouverte.',
  ...
  es: {
    ...
    mainTitle: 'Intercambios de Seguidores Pinterest',
    mainDesc: 'Siga perfiles y tableros de Pinterest de otros miembros para ganar saldo. Es obligatorio permanecer 15 segundos en navegación.',
    noCampaigns: 'No hay campañas de Seguidores Pinterest activas',
    noCampaignsDesc: 'Lanza tu nueva promoción para sumar seguidores Pinterest auténticos en minutos.',
    yourCampaign: 'Su campaña ⭐️',
    rewardPoints: '{reward} Puntos',
    requiredTime: '15 segundos',
    startLiking: 'Seguir Perfil & Gagner {reward} Pts',
    testAndPreview: 'Vérifier ma campagne & Aperçu',
    prevBtn: 'Précédent &rarr;',
    nextBtn: '&larr; Suivant',
    pageIndicator: 'Page {current} de {total}',
    modalTitle: 'Seguir en Pinterest',
    modalDesc: 'Intercambio de seguidores de Pinterest. Por favor espere en la ventana abierta de arriba.',
...
export default function FollowPinterest({ user, onPointsEarned, lang = 'en' }: FollowPinterestProps) {
  ...
  const t = pinFollowTranslations[activeLang];
  ...
  const loadCampaigns = () => {
    const list = db.getCampaigns()
      .filter(c => c.type === 'pinterest_follow' && c.status === 'active')
  ...
  const handleVerificationSuccess = (profileHandle: string) => {
    ...
    const success = db.recordInteraction(user.uid, activeCamp!.id, 'pinterest_follow', reward, profileHandle);
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {currentCampaigns.map((camp) => (
              <div id={`pin-follow-card-${camp.id}`} key={camp.id} className="bg-slate-900 border border-slate-800 hover:border-red-500/30 rounded-2xl p-5 flex flex-col justify-between gap-4 transition-all hover:translate-y-[-2px]">
                <div>
                ...
                >
                  <UserPlus className="w-4 h-4 text-red-500 hover:text-white" />
