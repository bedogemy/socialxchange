...
import { UserPlus, Clock, Award, AlertCircle, Sparkles, X, ShieldAlert, Info } from 'lucide-react';
import TaskVerificationOverlay from './TaskVerificationOverlay';
import { SupportedLanguages } from '../lib/translations';

interface FollowXProps {
  user: User;
  onPointsEarned: () => void;
  lang?: string;
}

const xFollowTranslations: Record<SupportedLanguages, Record<string, string>> = {
  ar: {
    availableTitle: 'متاح حالياً: {count} حملة',
    mainTitle: 'تبادل متابعات منصة إكس (Twitter)',
    mainDesc: 'شارك بمتابعة حسابات الأعضاء الفعالة على منصة إكس (تويتر) لجمع نقاط دعم سريعة. يرجى البقاء 15 ثانية لتخطي الأمان الزمني بنجاح.',
    noCampaigns: 'لا توجد حملات متابعة إكس حالياً',
    ...
    modalTitle: 'مهمة متابعة إكس (X)',
    modalDesc: 'تبادل متابعة وتفاعلات منصة إكس. يرجى الانتظار وتكمل التفاعل بالنافذة المفتوحة.',
  ...
  en: {
    ...
    mainTitle: 'X (Twitter) Follow Exchanges',
    mainDesc: 'Participate by following accounts of other members on X (Twitter) to collect instant points. Staying for 15 seconds is mandatory to verify authenticity.',
    noCampaigns: 'No X follow campaigns currently',
    noCampaignsDesc: 'Create your first X follow campaign now and thousands of members will react to it immediately!',
    ...
    modalTitle: 'X (Twitter) Follow Task',
    modalDesc: 'X follow and interaction exchange. Please wait and complete the action in the opened window.',
  ...
  fr: {
    ...
    mainTitle: 'Échanges d\'Abonnements X (Twitter)',
    mainDesc: 'Suivez les comptes d\'autres membres sur X (Twitter). Un délai de 15 secondes de présence continue est requis.',
    noCampaigns: 'Aucun objectif d\'Abonnement X',
    noCampaignsDesc: 'Créez votre première campagne X Follow maintenant pour obtenir de l\'audience instantanée !',
    ...
    modalTitle: 'S\'abonner sur X',
    modalDesc: 'Échange d\'abonnements X. Veuillez patienter dans la fenêtre externe ouverte.',
  ...
  es: {
    ...
    mainTitle: 'Intercambios de Seguidores X (Twitter)',
    mainDesc: 'Siga cuentas de la comunidad de X (Twitter) para ganar saldo. Es obligatorio permanecer 15 segundos en navegación.',
    noCampaigns: 'No hay campañas de Seguidores X activas',
    ...
    modalTitle: 'Acción Seguir X',
    modalDesc: 'Intercambio de suscripciones de cuentas X. Complete la interacción en el destino abierto.',
...
export default function FollowX({ user, onPointsEarned, lang = 'en' }: FollowXProps) {
  ...
  const t = xFollowTranslations[activeLang];
  ...
  const handleVerificationSuccess = (profileHandle: string) => {
    ...
    const success = db.recordInteraction(user.uid, activeCamp!.id, 'x_follow', reward, profileHandle);
    ...
    <div className={`w-full font-sans ${activeLang === 'ar' ? 'text-right' : 'text-left'}`} dir={activeLang === 'ar' ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between border-b border-slate-800 pb-5 mb-6 flex-row">
        ...
        <h2 className="text-xl md:text-2xl font-black text-white flex items-center gap-2 flex-row">
          {t.mainTitle}
          {/* Custom vector X Logo */}
          <span className="w-6 h-6 flex items-center justify-center font-black bg-white text-slate-950 text-xs rounded-md">X</span>
        </h2>
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {currentCampaigns.map((camp) => (
              <div id={`x-follow-card-${camp.id}`} key={camp.id} className="bg-slate-900 border border-slate-800 hover:border-slate-500/30 rounded-2xl p-5 flex flex-col justify-between gap-4 transition-all hover:translate-y-[-2px]">
                <div>
              ...
              <div className="flex items-center gap-2 flex-row">
                ...
                <span className="text-[11px] text-slate-400 font-bold block">
                  {lang === 'ar' 
                    ...
                    : '1. Click open target button below to visit and follow.'}
                </span>
              </div>
              <div className="flex items-center gap-2 flex-row">
                <Info className="w-4 h-4 text-slate-400 flex-shrink-0" />
