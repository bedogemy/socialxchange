...
import { UserPlus, Clock, Award, AlertCircle, Sparkles, X, ShieldAlert, Instagram } from 'lucide-react';
...
const igFollowTranslations: Record<SupportedLanguages, Record<string, string>> = {
  ar: {
    availableTitle: 'متاح حالياً: {count} حملة',
    mainTitle: 'تبادل متابعات انستغرام',
    mainDesc: 'تابع حسابات الآخرين على منصة انستغرام لجمع نقاط مكافأة فورية سريعة. يرجى الانتظار بالرابط المفتوح لـ 15 ثانية لتأكيد مصداقيتك.',
    noCampaigns: 'لا توجد حملات متابعة انستغرام حالياً',
    ...
    modalTitle: 'مهمة متابعة انستغرام',
    modalDesc: 'تبادل متابعات على انستغرام. يرجى الانتظار وتكمل التفاعل بالنافذة المفتوحة.',
  ...
  en: {
    ...
    mainTitle: 'Instagram Follow Exchanges',
    mainDesc: 'Participate by following accounts of other members on Instagram to collect instant points. Staying for 15 seconds is mandatory to verify authenticity.',
    noCampaigns: 'No Instagram follow campaigns currently',
    noCampaignsDesc: 'Create your first instagram follow campaign now and thousands of members will react to it immediately!',
    ...
    modalTitle: 'Instagram Follow Task',
    modalDesc: 'Instagram follow and interaction exchange. Please wait and complete the action in the opened window.',
  ...
  fr: {
    ...
    mainTitle: 'Échanges d\'Abonnements Instagram',
    mainDesc: 'Suivez d\'autres comptes sur Instagram pour accumuler des bonus de points. Un délai de 15 secondes d\'attention est requis.',
    noCampaigns: 'Aucun objectif d\'Abonnement Instagram',
    noCampaignsDesc: 'Créez votre première campagne Instagram Follow maintenant pour obtenir de l\'audience instantanée !',
    yourCampaign: 'Votre campagne ⭐️',
    rewardPoints: '{reward} Points',
    requiredTime: '15 secondes',
    startLiking: 'Suivre le profil & Gagner {reward} Pts',
    testAndPreview: 'Vérifier ma campagne & Aperçu',
    prevBtn: 'Précédent &rarr;',
    nextBtn: '&larr; Suivant',
    pageIndicator: 'Page {current} de {total}',
    modalTitle: 'S\'abonner sur Instagram',
    modalDesc: 'Échange d\'abonnements Instagram. Veuillez patienter dans la fenêtre externe ouverte.',
  ...
  es: {
    ...
    mainTitle: 'Intercambios de Seguidores Instagram',
    mainDesc: 'Siga cuentas de la comunidad de Instagram para ganar saldo. Es obligatorio permanecer 15 segundos en navegación.',
    noCampaigns: 'No hay campañas de Seguidores Instagram activas',
    ...
    startLiking: 'Seguir cuenta & Ganar {reward} Puntos',
    testAndPreview: 'Probar y Validar Campaña',
    prevBtn: 'Anterior &rarr;',
    nextBtn: '&larr; Siguiente',
    pageIndicator: 'Página {current} de {total}',
    modalTitle: 'Acción Seguir Instagram',
    modalDesc: 'Intercambio de suscripciones de perfiles Instagram. Complete la interacción en el destino abierto.',
    ...
    startAlert: '⚠️ Oprima el botón del centro para abrir la cuenta objetivo del anunciante',
    verificationError: 'Surgió un error. Su interacción no puede ser validada nuevamente.'
  }
};

export default function FollowInstagram({ user, onPointsEarned, lang = 'en' }: FollowInstagramProps) {
  ...
  const t = igFollowTranslations[activeLang];
  ...
  const handleVerificationSuccess = (profileHandle: string) => {
    ...
    const success = db.recordInteraction(user.uid, activeCamp!.id, 'ig_follow', reward, profileHandle);
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {currentCampaigns.map((camp) => (
              <div id={`ig-follow-card-${camp.id}`} key={camp.id} className="bg-slate-900 border border-slate-800 hover:border-pink-500/30 rounded-2xl p-5 flex flex-col justify-between gap-4 transition-all hover:translate-y-[-2px]">
                <div>
                ...
                <button
                  id={`ig-follow-start-${camp.id}`}
                  onClick={() => handleStart(camp)}
                  className="w-full py-2.5 bg-slate-950 hover:bg-pink-600 hover:text-white border border-slate-800 hover:border-pink-500 rounded-xl font-semibold text-xs text-pink-500 transition flex items-center justify-center gap-1.5 cursor-pointer flex-row"
                >
                  <UserPlus className="w-4 h-4 text-pink-500 hover:text-white" />
